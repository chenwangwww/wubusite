<template>
  <aside class="w-full">
    <div v-if="isDropdownOpen" class="fixed inset-0 bg-black/20 z-40" @click="isDropdownOpen = false"></div>

    <div class="relative z-50" @click="toggleDropdown">
      <div class="flex items-center gap-4 px-5 py-3 cursor-pointer border" :class="[
      ...
      <transition name="slide-fade">
        <div v-if="isDropdownOpen"
          class="absolute top-full left-0 w-full z-10 bg-white shadow-lg border-t-0 rounded-b-lg">
          <nav class="flex flex-col">
            <div v-for="item in dashboardItems" :key="item.name" @click.stop="setActiveAndClose(item)"
              class="flex items-center gap-4 px-5 py-3 cursor-pointer transition-all duration-200" :class="[
            ...
            <div v-for="item in settingItems" :key="item.name" @click.stop="setActiveAndClose(item)"
              class="flex items-center gap-4 px-5 py-3 cursor-pointer transition-all duration-200" :class="[
                activeItem === item.name
                  ? 'bg-[#fffbf8] border-l-[5px] border-[#ff7445] text-[#ff7445]'
                  : 'text-gray-600 hover:bg-gray-100 border-l-[5px] border-white'
            ...
            <div v-for="item in accountItems" :key="item.name" @click.stop="setActiveAndClose(item)"
              class="flex items-center gap-4 px-5 py-3 cursor-pointer transition-all duration-200" :class="[
                activeItem === item.name
                  ? 'bg-[#fffbf8] border-l-[5px] border-[#ff7445] text-[#ff7445]'
                  : 'text-gray-600 hover:bg-gray-100 border-l-[5px] border-white'
  ...
  <RegularPop v-if="showCloseAccount" title="Close account" body-text="Are You sure you want to close your account?"
    confirm-text="Yes" :showCancelButton="true" cancelText="No" @confirm="handleConfirm" @cancel="handleConfirm">
  </RegularPop>
</template>
